        ...
        <div class="sidebar pe-4 pb-3">
            <nav class="navbar bg-secondary navbar-dark">
                ...
                <div class="navbar-nav w-100">
                    <a href="home.php" class="nav-item nav-link active"><i class="fa fa-home me-2"></i>Home</a>
                    ...
                    <div class="table-responsive">
                        <!-- <?php
                            if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['id'])) {
                                $id = intval($_POST['id']); // Validate the ID to prevent SQL injection

                                // Fetch the image path before deleting the record
                                $selectImageSql = "SELECT image FROM singin_database_name WHERE id = ?";
                                if ($stmt = $conn->prepare($selectImageSql)) {
                                    ...
                                    // Delete the record
                                    $deleteSql = "DELETE FROM singin_database_name WHERE id = ?";
                                    if ($stmt = $conn->prepare($deleteSql)) {
                                        ...
                                        if ($stmt->execute()) {
                                            ...
                                            echo "<div class='alert alert-success'>User with ID $id and associated image deleted successfully.</div>";
                                        } else {
                                    ...
                                    echo "<div class='alert alert-danger'>Error fetching image path: " . $conn->error . "</div>";
                                }
                            }
                        ?>-->
                        ...
                        <table class="table text-start align-middle table-bordered table-hover mb-0">
                            ...
                            <tbody>
                                ...
                                $sql = "SELECT id, username, email, password FROM singin_database_name";
                ...
                <div class="row g-4">
                    <div class="col-sm-12 col-md-6 col-xl-8">
                        <div class="h-100 bg-secondary rounded p-4">
                            ...
                            <?php
                                $email_sql = "SELECT mail FROM email LIMIT 10";
                                $email_result = $conn->query($email_sql);

                                if ($email_result->num_rows > 0) {
                                    while ($email_row = $email_result->fetch_assoc()) {
                                        echo '<div class="d-flex align-items-center border-bottom py-2">
                                                <h5 class="me-auto">' . htmlspecialchars($email_row['mail']) . '</h5>
                                                <form method="post" action="home.php" class="ms-3">
                                                    <input type="hidden" name="email" value="' . htmlspecialchars($email_row['mail']) . '">
                                ...
                                if ($_SERVER['REQUEST_METHOD'] === 'POST') {
                                    if (isset($_POST['email'])) {
                                        ...
                                        $stmt = $conn->prepare("DELETE FROM email WHERE mail = ?");
