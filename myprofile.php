    ...
    <div class="container-fluid position-relative d-flex p-0">
            ...
            <nav class="navbar bg-secondary navbar-dark">
                        ...
                        <?php
                            ...
                            if (isset($_SESSION['username'])) {
                                ...
                                if ($result->num_rows > 0) {
                                    ...
                                    while ($row = $result->fetch_assoc()) {
                                        // Display the image
                                        echo "<img class='rounded-circle me-lg-2 border border-2 border-white' src='" . htmlspecialchars($row['image']) . "' alt='' style='width: 50px; height: 50px; object-fit: cover;'>";
                ...
                <div class="navbar-nav w-100">
                    ...
                    <div class="nav-item dropdown">
                        <a href="#" class="nav-link dropdown-toggle" data-bs-toggle="dropdown"><i class="fa fa-bars me-2"></i>All Products</a>
        ...
        <div class="content">
            ...
            <div class="container-fluid pt-4 px-4 ">
                <div class="row sa bg-secondary rounded  justify-content-center mx-0" style="height: 73vh;">
                    <div class="col-md-6 text-center mt-5">
                        <form method="POST" action="myprofile.php" enctype="multipart/form-data" class="mt-3">
                            <div style="position: relative; display: inline-block; width: 200px; height: 200px;">
                                ...
                                if (isset($_SESSION['username'])) {
                                    ...
                                    if ($result->num_rows > 0) {
                                        ...
                                        while ($row = $result->fetch_assoc()) {
                                            // Display the image
                                            echo "<img id='profileImage' class='rounded-circle me-lg-2 border border-2 border-white' src='" . htmlspecialchars($row['image']) . "' alt='' style='width: 200px; height: 200px; object-fit: cover;'>";
                                        }
                                    } else {
                                        echo "No user found";
                                    }
                                ...
                                ?>
                                <!-- File Input -->
                                <input type="file" id="imageInput" name="new_profile_image" accept="image/*" class="form-control" style="display: none;" onchange="this.form.submit();" required>
                                <!-- Edit Button -->
                        ...
                        <h4>Email:
                            ...
                            if (isset($_SESSION['username'])) {
                                ...
                                $stmt = $conn->prepare("SELECT email FROM singin_database_name WHERE username = ?");
                                $stmt->bind_param("s", $_SESSION['username']);
                                $stmt->execute();
                                $result = $stmt->get_result();
                                ...
                                if ($result->num_rows > 0) {
                                    // Output email
                                    while ($row = $result->fetch_assoc()) {
                                        echo "<span class='d-none d-lg-inline-flex'>" . htmlspecialchars($row["email"]) . "</span>";
                                    }
                                } else {
                                    echo "No user found";
                                }
                        ...
                        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
                            if (isset($_FILES['new_profile_image']) && $_FILES['new_profile_image']['error'] === UPLOAD_ERR_OK) {
                                ...
                                if (isset($_SESSION['username'])) {
                                    ...
                                    $stmt = $conn->prepare("SELECT image FROM singin_database_name WHERE username = ?");
                                    $stmt->bind_param("s", $_SESSION['username']);
                                    $stmt->execute();
                                    $result = $stmt->get_result();
                                    ...
                                    if ($result->num_rows > 0) {
                                        ...
                                        $upload_dir = "/Admin/image_upload/"; // Change this to your upload directory
                                        $file_name = basename($_FILES['new_profile_image']['name']);
                                        $file_path = $upload_dir . $file_name;

                                        if (move_uploaded_file($_FILES['new_profile_image']['tmp_name'], $_SERVER['DOCUMENT_ROOT'] . $file_path)) {
                                            // Update the image path in the database
                                            $update_stmt = $conn->prepare("UPDATE singin_database_name SET image = ? WHERE username = ?");
                                            $update_stmt->bind_param("ss", $file_path, $_SESSION['username']);
                                            if ($update_stmt->execute()) {
                                                echo "<script>alert('Profile image updated successfully'); window.location.href = window.location.href;</script>";
                                            } else {
                                    ...
                                    echo "<script>alert('No session username found');</script>";
                                }
                            } else {
